...
@section('content')
    <div class="text-center" style="margin: 0 auto;">
        <div class="form-inline">
            <label for="ship-name" >舰船名称：</label>
            <input type="text" id="ship-name" class="form-control" placeholder="如：Lexington"/>
            <label for="ship-no">舰船编号：</label>
            <input type="text" id="ship-no" class="form-control" placeholder="如：CV-2"/>
            <label for="ship-type">舰船类型：</label>
            <input type="text" id="ship-type" class="form-control" placeholder="如：Aircraft"/>
            <label for="ship-country">舰船国籍：</label>
            <input type="text" id="ship-country" class="form-control" placeholder="如：American"/>
            <button id="btn-find" type="button" class="btn btn-primary">搜索</button>
        </div>
    </div>
    ...
    <script type="text/javascript">
         ...
         * 宽度 : 高度 = 200 * 320
         */
        var mainWidth = 800; // 图片宽度
        var mainMargin = 2 * 10; // margin 填充宽度
        var x; // 图片横向坐标
        var y; // 图片纵向坐标
        var MAX_X = 0; // 图片横向最大行数
        var MAX_Y = 1; // 图片纵向最大列数
        ...
        $(document).ready(function () {
            $('#btn-find').on('click',function () {
                ...
                var no = $('#ship-no').val();
                var type = $('#ship-type').val();
                var country = $('#ship-country').val();
                getWaarships(name,no,type,country);
            });
        });

        function getWaarships(name,no,type,country){
            $('#slipping-container-master').empty();
            $.ajax({
                type:'GET',
                ...
                success:function (data) {
                    ...
                    for(x = 1;x <= MAX_X ;++x){
                        ...
                        $("#slipping-container-" + x).css('height','310px'); // 在修改过显示规格后，需要手动设置横向载体容器元素高度
                        ...
                        for(y = 1;y <= MAX_Y;++y){
                            ...
                            if(( (x - 1) * MAX_Y + (y - 1) ) < totalNum) {
                                ...
                                $(".slipping-main-"+ x + y).append("<div class='slipping-box thumbnail'>" +
                                    "<a target='_blank' href='" + data[ (x - 1) * MAX_Y + (y - 1) ].pageUrl + "'>" +
                                    "<img src='" + data[ (x - 1) * MAX_Y + (y - 1) ].pictureUrl + "' style='width:800px;height:300px'>" +
                                    "</a></div>");
